...
#if LWIP_PTP
				__disable_irq ();
			  if(TXTS_RDY & PTPCheckForEvents(DP83848_PHY_ADDRESS))
					PTPGetTransmitTimestamp(DP83848_PHY_ADDRESS, (uint32_t*)(&time->seconds), (uint32_t*)(&time->nanoseconds), &overflowCount);
				while(TXTS_RDY & PTPCheckForEvents(DP83848_PHY_ADDRESS))
					PTPGetTransmitTimestamp(DP83848_PHY_ADDRESS, &sec_flush, &nsec_flush, &overflowCount);
				__enable_irq ();
#else
