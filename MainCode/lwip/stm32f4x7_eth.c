...
/** @defgroup ETH_Private_Functions
  * @{
  */
	static uint32_t ETH_Select_numpage(uint16_t PHYAddress,uint16_t numpage);
...
/*********************************specific for DP83640**************************/
uint32_t ETH_Select_numpage(uint16_t PHYAddress,uint16_t numpage) {
  ETH_WritePHYRegister(PHYAddress, PAGESEL, numpage);
	return ETH_SUCCESS;
}

uint32_t ETH_WritePHYRegister_p(uint16_t PHYAddress, uint16_t page, uint16_t PHYReg, uint16_t PHYValue) {
	ETH_Select_numpage(PHYAddress, page);
	ETH_WritePHYRegister(PHYAddress, PHYReg, PHYValue);
  return ETH_SUCCESS;
}

uint16_t ETH_ReadPHYRegister_p(uint16_t PHYAddress, uint16_t page, uint16_t PHYReg) {
	uint16_t tmpreg;
	ETH_Select_numpage(PHYAddress, page);
	tmpreg = ETH_ReadPHYRegister(PHYAddress, PHYReg);
	return tmpreg;
}

uint16_t tmpreg;

void PHY_DP83640_init(uint16_t PHYAddress) {
	uint32_t overflowCount, sec = 0, nsec = 0, sequenceId, hashValue;
	uint16_t /*tmpreg = 0*/ event = 0;
	uint8_t messageType;
	//disable ptp
	...
	//init ptp clock
	ETH_PTPTime_AdjFreq(0);
	PHY_setTime(PHYAddress, &sec, &nsec);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_COC_PAGE, PTP_COC);
	tmpreg |= (PTP_CLKOUT_EN | PTP_CLKOUT_SEL);
	ETH_WritePHYRegister_p(PHYAddress, PTP_COC_PAGE, PTP_COC, tmpreg);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_PHYCR2_PAGE, PHYCR2);
	tmpreg &= ~CLK_OUT_DIS;
	ETH_WritePHYRegister_p(PHYAddress, PTP_PHYCR2_PAGE, PHYCR2, tmpreg);
	//enable ptp
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL);
	tmpreg |= PTP_ENABLE;
	ETH_WritePHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL, tmpreg);
	...
	tmpreg = (TRIG_ON_AMP << TRIG_CSEL_SHIFT) | ((TRIG_ON_AMP + 2)  << TRIG_GPIO_SHIFT) | TRIG_PULSE | TRIG_WR | TRIG_NOTIFY;
	ETH_WritePHYRegister_p(PHYAddress, PTP_TRIG_PAGE, PTP_TRIG, tmpreg );	//trigger ON_AMP
	tmpreg = (TRIG_OF_HYD << TRIG_CSEL_SHIFT) | ((TRIG_OF_HYD + 2)  << TRIG_GPIO_SHIFT) | TRIG_PULSE | TRIG_WR | TRIG_NOTIFY;
	ETH_WritePHYRegister_p(PHYAddress, PTP_TRIG_PAGE, PTP_TRIG, tmpreg);	//trigger OF_HYD
	//disable Transmit and Receive Timestamp
	ETH_WritePHYRegister_p(PHYAddress, PTP_TXCFG_PAGE, PTP_TXCFG0, 0);
	ETH_WritePHYRegister_p(PHYAddress, PTP_TXCFG_PAGE, PTP_TXCFG1, 0);
	ETH_WritePHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG0, 0);
	ETH_WritePHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG1, 0);
	ETH_WritePHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG3, 0);	
	ETH_WritePHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG4, 0);
	//flush Transmit and Receive Timestamps
	while(event = PTPCheckForEvents(PHYAddress)) {
		if(event & TXTS_RDY)
			PTPGetTransmitTimestamp(PHYAddress, &sec, &nsec, &overflowCount);
		else if ( event & RXTS_RDY)
			PTPGetReceivTimestamp(PHYAddress, &sec, &nsec, &overflowCount, &sequenceId, &messageType, &hashValue);
	}
	//init transmit part
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_TXCFG_PAGE, PTP_TXCFG0);
	tmpreg |= (TX_IPV4_EN | TX_PTP_VER_2 | TX_TS_EN | IP1588_EN | IGNORE_2STEP);
	ETH_WritePHYRegister_p(PHYAddress, PTP_TXCFG_PAGE, PTP_TXCFG0, tmpreg);
	ETH_WritePHYRegister_p(PHYAddress, PTP_TXCFG_PAGE, PTP_TXCFG1, /*0xFF00*/ 0);
	//init receive part
	ETH_WritePHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG1, /*0xFF00*/ 0);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG0);
	tmpreg |= (RX_IPV4_EN | RX_PTP_VER_2 | RX_TS_EN | IP1588_EN0 | IP1588_EN1 | IP1588_EN2);
	ETH_WritePHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG0, tmpreg);	
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG3);
	tmpreg |= (/*TS_INSERT |*/ ACC_UDP | ACC_CRC);
	ETH_WritePHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG3, tmpreg);	
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG0);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG1);
	//tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG2);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG3);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG4);
	/*tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG4);
	tmpreg |= (TS_SEC_EN | RXTS_NS_OFF_16 | RXTS_SEC_OFF_5);
	ETH_WritePHYRegister_p(PHYAddress, PTP_RXCFG_PAGE, PTP_RXCFG4, tmpreg);	*/
	return;
}

void PHY_getTime(uint16_t PHYAddress, s32_t* sec, s32_t* nsec) {
	uint16_t tmpreg = 0;
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL);
	tmpreg |= PTP_RD_CLK;
	ETH_WritePHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL, tmpreg);
	*nsec = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	*((int16_t*)nsec + 1) = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	*sec = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	*((int16_t*)sec + 1) = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	return;
}

void PHY_setTime(uint16_t PHYAddress, uint32_t *sec, uint32_t *nsec) {
	uint16_t tmpreg = 0;
	ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, *nsec & 0xFFFF);
	ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, *nsec >> 16);
	ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, *sec & 0xFFFF);
	ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, *sec >> 16);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL);
	tmpreg |= PTP_LOAD_CLK;
	ETH_WritePHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL, tmpreg);
	return;
...
uint16_t PTPCheckForEvents(uint16_t PHYAddress) {
	uint16_t tmpreg = 0;
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_STS_PAGE, PTP_STS);
	return (tmpreg & MASK_EVENT_FL);
}

void PTPGetTransmitTimestamp(uint16_t PHYAddress, uint32_t *sec, uint32_t *nsec, uint32_t *overflowCount) {
	uint16_t tmpreg = 0;
	*nsec = ETH_ReadPHYRegister_p(PHYAddress, PTP_TXTS_PAGE, PTP_TXTS);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_TXTS_PAGE, PTP_TXTS);
	*overflowCount = (tmpreg & 0xC000) >> 14;
	*nsec |= (tmpreg & 0x3FFF) << 16;
	*sec = ETH_ReadPHYRegister_p(PHYAddress, PTP_TXTS_PAGE, PTP_TXTS);
	*sec |= ETH_ReadPHYRegister_p(PHYAddress, PTP_TXTS_PAGE, PTP_TXTS)<<16;
	return;
}

void PTPGetReceivTimestamp(uint16_t PHYAddress, uint32_t *sec, uint32_t *nsec, uint32_t *overflowCount, 
                             uint32_t *sequenceId, uint8_t *messageType, uint32_t *hashValue) {
	uint16_t tmpreg = 0;
	*nsec = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXTS_PAGE, PTP_RXTS);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXTS_PAGE, PTP_RXTS);
	*overflowCount = (tmpreg & 0xC000) >> 14;
	*nsec |= (tmpreg & 0x3FFF) << 16;
	*sec = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXTS_PAGE, PTP_RXTS);
	*sec |= ETH_ReadPHYRegister_p(PHYAddress, PTP_RXTS_PAGE, PTP_RXTS)<<16;
	*sequenceId = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXTS_PAGE, PTP_RXTS);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_RXTS_PAGE, PTP_RXTS);
	*messageType = tmpreg >> 12;
  *hashValue = tmpreg & 0x0FFF;
	return;
}

void getReceivTimestamp(uint16_t PHYAddress, uint8_t mes, uint32_t seq,  int32_t *sec, int32_t *nsec) {
	uint8_t messageType = 0;
	uint32_t overflowCount = 0, hashValue = 0, sequenceId = 0;
	if((mes != SYNC) && (mes != DELAY_REQ) && (mes != PDELAY_REQ) && (mes != PDELAY_RESP)) {
		PHY_getTime(PHYAddress, (s32_t*)sec, (s32_t*)nsec);
		return;
	}
	while((PTPCheckForEvents(PHYAddress)) & RXTS_RDY ) {
		PTPGetReceivTimestamp(PHYAddress, (uint32_t*)sec, (uint32_t*)nsec, &overflowCount, &sequenceId, &messageType, &hashValue);
		/*if(overflowCount)
			seterror(ER_PTPRS_OVER_FIFO);*/
		if((seq == sequenceId) && (mes == messageType)) {
			return;
		}
		/*else
			seterror(ER_PTPRS_MIS_EVENT);*/
	}
//	seterror(ER_PTPRS_EMPTY_FIFO);
	PHY_getTime(PHYAddress, (s32_t*)sec, (s32_t*)nsec);
	return;
}

void PTPArmTrigger(uint16_t PHYAddress, uint32_t trig, uint32_t sec, uint32_t nsec, uint32_t pulseWidth) {
	uint16_t tmpreg = 0;
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL); 
	tmpreg = (trig << TRIG_SEL_SHIFT) | TRIG_LOAD;
	ETH_WritePHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL, tmpreg );
	tmpreg = nsec & 0xFFFF;
	ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, tmpreg);
	tmpreg = nsec >> 16;
	ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, tmpreg);
	tmpreg = sec & 0xFFFF;
	ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, tmpreg);
	...
	ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, tmpreg); 
	if(pulseWidth) {
		tmpreg = pulseWidth & 0xFFFF;
		ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, tmpreg);
		tmpreg = pulseWidth >> 16;
	  ETH_WritePHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR, tmpreg); 
	}
	tmpreg = (trig << TRIG_SEL_SHIFT) | TRIG_EN;
	ETH_WritePHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL, tmpreg);
	return;
}

void PTPgetArmTrigger(uint16_t PHYAddress, uint32_t trig, uint32_t* sec, uint32_t* nsec, uint32_t* pulseWidth) {
	uint16_t tmpreg = 0;
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL); 
	tmpreg = (trig << TRIG_SEL_SHIFT) | TRIG_READ;
	ETH_WritePHYRegister_p(PHYAddress, PTP_CTL_PAGE, PTP_CTL, tmpreg );
	
	*nsec = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	*nsec |= (tmpreg << 16);
	*sec = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	*sec |= (tmpreg << 16);
	*pulseWidth = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	tmpreg = ETH_ReadPHYRegister_p(PHYAddress, PTP_TDR_PAGE, PTP_TDR);
	*pulseWidth |= (tmpreg << 16);	
